...
export async function GET() {
  try {
    ...
    const queriesUsed = usageRecord?.query_count || 0;
    const totalTokensToday = usageRecord?.total_tokens || 0;
    const queriesRemaining = Math.max(0, tierConfig.dailyQueries - queriesUsed);
    ...
    return NextResponse.json({
      ...
      tier,
      message: getQueryLimitMessage(tier, queriesUsed),
