...
function SignupForm() {
  ...
  const { signup, isAuthenticated, upgradeTier } = useAuth();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    // Always sign up as normie first (free trial)
    const result = await signup(email, password, 'normie');
    if (result.success) {
      // If they selected a paid tier, redirect to Stripe checkout
      if (selectedTier !== 'normie') {
        await upgradeTier(selectedTier);
        // upgradeTier redirects to Stripe, so we don't push to /
      } else {
        router.push('/');
      }
    } else {
